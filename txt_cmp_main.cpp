...
int main(int argc, char** argv)
{
	...
	/* 进入实际的功能调用，完成相应的功能 */
	string file1_name = args[OPT_ARGS_FILE1].get_string();
	string file2_name = args[OPT_ARGS_FILE2].get_string();

	Cmp txt_compare;
	txt_compare.openFile(file1_name, file2_name);
	if (args[OPT_ARGS_TRIM].existed())
		txt_compare.trim_type = args[OPT_ARGS_TRIM].get_string(); //trim
	if (args[OPT_ARGS_LINE_SKIP].existed())
		txt_compare.lineskip = args[OPT_ARGS_LINE_SKIP].get_int(); //lineskip
	if (args[OPT_ARGS_LINE_OFFSET].existed())
		txt_compare.lineoffset = args[OPT_ARGS_LINE_OFFSET].get_int();//lineoffset
	if (args[OPT_ARGS_IGNORE_BLANK].existed())
		txt_compare.ignore_blank = 1;								//ignore_blank
	if (args[OPT_ARGS_CR_CRLF_NOT_EQUAL].existed())
		txt_compare.CR_CRLF_not_equal = 1;							//CR_CRLF_not_equal
	if (args[OPT_ARGS_MAX_DIFFNUM].existed())
		txt_compare.max_diff = args[OPT_ARGS_MAX_DIFFNUM].get_int();//max diff
	if (args[OPT_ARGS_MAX_LINENUM].existed())
		txt_compare.max_line = args[OPT_ARGS_MAX_LINENUM].get_int();//max_line
	if (args[OPT_ARGS_DISPLAY].existed())
		txt_compare.display = args[OPT_ARGS_DISPLAY].get_string();//display


	txt_compare.fileCmp();
	txt_compare.closeFile();
